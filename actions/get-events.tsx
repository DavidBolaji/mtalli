...
export const filterEvent = (
  ...
  const sort =
    (formData.get("sort") as string) || params.get("sort") || "title"; // Default to "name"
  const sortbooking =
    (formData.get("sortbooking") as string) ||
    params.get("sortbooking") ||
    "asc";
...
export const getEventsByQuery = async (search: string): Promise<IEvent[]> => {
  try {
    const events = await db.event.findMany({
      where: {
        title: {
