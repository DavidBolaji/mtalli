...
export default function AddEvent({ event }: { event?: IEvent | null }) {
  ...
  useEffect(() => {
    rerender(isEdit, queryClient, event, key, urls);
  }, [event]);
    ...
    mutationFn: async () => {
      ...
      allEventSchema
        ...
        .then(async () => {
          let data: any;
          if (isEdit) {
            data = await Axios.put(`/event`, event);
          } else {
            data = await Axios.post("/event", event);
          }
          toggleNotification({
            show: true,
  ...
  const reset = (eventz?: any) => {
    queryClient.setQueryData([key], eventz);
  };
    ...
    <div className="container mx-auto mt-6 overflow-hidden">
      ...
      <div className="grid gap-6 grid-cols-2">
        ...
        <Card className="px-4 pt-6 pb-10 col-span-2">
          <div className="flex justify-between">
            <Typography
              ...
              as="p"
              align="left"
              className="mb-4 font-bold text-md "
            >
              Event Details
            </Typography>
        ...
        <Card className="px-4 pt-6 col-span-2">
          ...
          <UploadImageForm<IcreateEvent> keyz={key} count={5} urls={urls} />
        </Card>
